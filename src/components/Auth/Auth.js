...
export function Auth({unsplashState, setUnsplashState}) {
  ...
  useEffect(() => {
    setTimeout(goToRoot(),5000);//выполнился вхолостую ибо нет ключа в стейте. Но далее, когда выполнится след ф = обновится unsplashState = выполнится заново и сработает.
