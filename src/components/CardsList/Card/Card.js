...
export function Card({add,url, created, likes, handlePressHeart, profile, name, ava, description, open, id, getChosenImage, pressed, setPressed, setLikedId, likePhoto, isLiked, isAuth}) {
  return (
    <div className={styles.card}>
      <Info created={created} profile={profile} name={name} ava={ava} description={description}/>
      <Link to={'/cardpage'} onClick={()=>getChosenImage(id)} >
        <Preview url={url}/>
      </Link>
